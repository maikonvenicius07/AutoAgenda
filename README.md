...
## V3.8.8 — Limpeza e organização do projeto

A V3.8.8 reorganiza o repositório sem alterar regras de negócio. Arquivos antigos da raiz foram removidos, a documentação histórica foi centralizada em `docs/historico/`, os procedimentos operacionais em `docs/operacao/` e o planejamento em `docs/planejamento/`. O `.gitignore` também foi restaurado para reduzir o risco de versionar segredos, dependências, logs e arquivos locais.

## V3.8.7 — Cancelamento rápido na Agenda de hoje

- botão **❌ Cancelar** diretamente no cartão da aula em **Agenda de hoje**;
- confirmação antes de cancelar;
- usa a rota específica `PATCH /api/aulas/:id/status` com `CANCELADA`;
- libera o horário imediatamente e preserva a aula no histórico;
- disponível para ADMIN e para o Instrutor responsável pela própria aula;
- após cancelar, oferece procurar um horário de reposição.

## V3.8.6 — Cancelamento direto e definitivo

- Cancelar uma aula agora usa um caminho transacional próprio no backend.
- O cancelamento não depende de saldo, conflito, disponibilidade, funcionamento ou validações de agenda que não são necessárias para liberar o horário.
- A ocorrência cancelada permanece no histórico e, quando pertence a plano automático, fica marcada como exceção do plano.
- Tokens de confirmação são invalidados ao cancelar.
- Comunicações de cancelamento são disparadas somente depois do `COMMIT`, sem bloquear a operação principal.
- Erros internos de status podem devolver código diagnóstico seguro para facilitar suporte.
- 108/108 testes automáticos aprovados.
...
A V3.8.3 corrige a exibição da confirmação feita pelo aluno por meio do link enviado no WhatsApp. A consulta principal de aulas passa a devolver `confirmacao_status`, `confirmacao_origem` e `confirmacao_atualizada_em`, permitindo que a Agenda e a Agenda de hoje mostrem o estado real gravado no PostgreSQL.
...
Consulte `docs/historico/CHECKLIST_V3.7.0.md` e `docs/operacao/CONFIGURAR_AUTOMACAO_V3.7.0.md` antes da ativação real.
...
Consulte `docs/historico/CHECKLIST_V3.5.0.md` antes de restaurar dados reais.
