...
test('status e saldo normalizam DATE do PostgreSQL antes de comparar', server.includes('const data = isoDateUTC(dateOnlyUTC(dataAula));') && server.includes('return isoDateUTC(dateOnlyUTC(dataAula)) >= hojeApp();'));
test('ADMIN usa rota específica quando altera somente situação/confirmação da aula', app.includes('dadosEstruturaisAulaIguais') && app.includes("await api(`/api/aulas/${id}/status`") && app.includes('Aula cancelada. O horário foi liberado.'));
test('cancelamento possui caminho simples sem validar saldo/conflito/disponibilidade', server.includes("if (status === 'CANCELADA')") && server.includes("SET status='CANCELADA'") && server.includes('excecao_plano=CASE WHEN plan_id IS NULL THEN excecao_plano ELSE TRUE END'));
test('Agenda de hoje possui cancelamento rápido com confirmação', app.includes('data-cancel-aula-rapido') && app.includes('cancelarAulaRapido') && app.includes("body:JSON.stringify({ status:'CANCELADA' })") && app.includes('Aula cancelada. O horário foi liberado.'));
